...
def _sudo_write_file(path, content):
    """Write content to a root-owned path via sudo cp from a temp file.

    Avoids piping content to sudo tee (which ties up stdin and prevents
    sudo from prompting for a password when the session is not cached).
    Returns (success, stderr_str).
    """
...
class SharedFoundationInfrastructureAdapter(INESDataInfrastructureAdapter):
    ...
    def _sync_nginx_http_proxy_vm_distributed(self):
        ...
        ok, err = _sudo_write_file(conf_path, conf)
        if not ok:
            print(f"Warning: could not write {conf_path}: {err}")
            return
    ...
    def _patch_dataspace_nginx_for_vm_distributed(self, deployer_config, ds_name, ds_domain):
        ...
        # Always load both adapter configs so nginx is generated for both simultaneously.
        sibling = self._load_sibling_adapter_configs(deployer_config)
        inesdata_cfg = sibling.get("inesdata") or {}
        edc_cfg      = sibling.get("edc") or {}

        inesdata_ds      = inesdata_cfg.get("ds_name") or ds_name
        inesdata_pshort  = inesdata_cfg.get("provider_short") or "citycouncil"
        inesdata_cshort  = inesdata_cfg.get("consumer_short") or "company"
        provider_shorts  = [inesdata_pshort]
        consumer_shorts  = [inesdata_cshort]

        edc_ds      = edc_cfg.get("ds_name") or "pionera-edc"
        edc_pshort  = edc_cfg.get("provider_short") or "citycounciledc"
        edc_cshort  = edc_cfg.get("consumer_short") or "companyedc"
        ...
        # --- Connector locations ---
        inesdata_links = "".join(
            f'<li><a href=\\\"/c/{s}/inesdata-connector-interface/\\\">{s.capitalize()} (INESData)</a></li>'
            for s in (provider_shorts + consumer_shorts)
        )
        edc_links = (
            f'<li><a href=\\\"/c/{edc_pshort}/edc-dashboard/\\\">{edc_pshort.capitalize()} (EDC)</a></li>'
            f'<li><a href=\\\"/c/{edc_cshort}/edc-dashboard/\\\">{edc_cshort.capitalize()} (EDC)</a></li>'
        )
        conn_links = inesdata_links + edc_links

        def _connector_locations(shorts, backend_ip, backend_port, conn_ds_name):
            out = ""
            for short in shorts:
                conn_host = f"conn-{short}-{conn_ds_name}.{ds_domain}"
                out += (
        ...
        def _edc_dashboard_locations(edc_shorts_ips):
            """Generate EDC dashboard entry-point and cookie-routed proxy locations.

            edc_shorts_ips: list of (short, backend_ip, backend_port, edc_conn_host)
            The /edc-dashboard/ and /edc-dashboard-api/ locations use $edc_connector_backend
            and $edc_connector_host map variables defined in connector-routing.conf.
            """
            out = ""
            for short, _ip, _port, _host in edc_shorts_ips:
                out += (
                    f"    location ~* ^/c/{short}/edc-dashboard(.*)$ {{\n"
                    f'        add_header Set-Cookie "edc_connector={short}; Path=/; SameSite=Lax" always;\n'
                    f"        return 302 /edc-dashboard$1$is_args$args;\n"
                    f"    }}\n"
                )
            out += (
                "    location /edc-dashboard/ {\n"
                "        proxy_pass http://$edc_connector_backend;\n"
                "        proxy_set_header Host $edc_connector_host;\n"
                "        proxy_set_header X-Real-IP $remote_addr;\n"
                "        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;\n"
                "        proxy_set_header X-Forwarded-Proto $scheme;\n"
                "    }\n"
                "    location /edc-dashboard-api/ {\n"
                "        proxy_pass http://$edc_connector_backend;\n"
                "        proxy_set_header Host $edc_connector_host;\n"
                f"        proxy_set_header X-Forwarded-Host org1.{ds_domain};\n"
                "        proxy_set_header X-Forwarded-Proto $scheme;\n"
                "        proxy_set_header X-Real-IP $remote_addr;\n"
                "        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;\n"
                "    }\n"
        ...
        connector_locs  = _connector_locations(provider_shorts, provider_ip, provider_nodeport, inesdata_ds)
        connector_locs += _connector_locations(consumer_shorts, consumer_ip, consumer_nodeport, inesdata_ds)

        edc_shorts_ips = [
            (edc_pshort, provider_ip, provider_nodeport, f"conn-{edc_pshort}-{edc_ds}.{ds_domain}"),
            (edc_cshort, consumer_ip, consumer_nodeport, f"conn-{edc_cshort}-{edc_ds}.{ds_domain}"),
        ]
        edc_locs = _edc_dashboard_locations(edc_shorts_ips)
        ...
        conf = (
            ...
            "    }\n"
            "}\n\n"
            "# MinIO Console — WebSocket upgrade required for object listing\n"
            "server {\n"
            f"    listen {common_ip}:80;\n"
            f"    server_name console.minio-s3.{ds_domain};\n"
            "    client_max_body_size 0;\n"
            "    location / {\n"
            f"        proxy_pass http://{common_ip}:{nodeport};\n"
            "        proxy_set_header Host $host;\n"
            ...
            for suffix in (".https.json", ".json"):
                ...
                try:
                    ...
                    if changed:
                        ...
                        _sudo_write_file(cfg_path, patched)
                        print(f"Patched app.config {cfg_path}")
                except Exception as exc:
                    print(f"Warning: could not patch {cfg_path}: {exc}")

        ok, err = _sudo_write_file(DATASPACE_CONF, conf)
        if not ok:
            print(f"Warning: could not write {DATASPACE_CONF}: {err}")
            return
        reload = subprocess.run(["sudo", "nginx", "-s", "reload"], capture_output=True, text=True)
        if reload.returncode == 0:
    ...
    def _load_sibling_adapter_configs(self, deployer_config):
        """Load INESData and EDC adapter configs independently of which adapter is running.

        Returns a dict with keys 'inesdata' and 'edc', each containing ds_name,
        provider_short, and consumer_short.  Missing config files yield None for that key.
        """
        ...
        def _read_config(*paths):
            ...
            for p in paths:
                try:
                    text = _Path(p).read_text(encoding="utf-8")
                    parser = _cp.RawConfigParser()
                    parser.read_string("[root]\n" + text)
                    merged.update(dict(parser["root"]))
                except Exception:
                    pass
            return {k.upper(): v.strip() for k, v in merged.items()}

        try:
            project_root = _Path(__file__).resolve().parents[2]
        except Exception:
            return {"inesdata": None, "edc": None}

        # Infrastructure topology base (VM_PROVIDER_CONNECTORS=citycouncil, VM_CONSUMER_CONNECTORS=company)
        infra_topo = _read_config(
            project_root / "deployers/infrastructure/topologies/vm-distributed.config",
        )
        # INESData adapter
        inesdata_cfg = _read_config(project_root / "deployers/inesdata/deployer.config")
        inesdata_ds = inesdata_cfg.get("DS_1_NAME") or "pionera"
        inesdata_provider = infra_topo.get("VM_PROVIDER_CONNECTORS", "citycouncil").split(",")[0].strip()
        inesdata_consumer = infra_topo.get("VM_CONSUMER_CONNECTORS", "company").split(",")[0].strip()

        # EDC adapter (overrides VM_PROVIDER/CONSUMER_CONNECTORS in its topo file)
        edc_cfg = _read_config(
            project_root / "deployers/edc/deployer.config",
            project_root / "deployers/edc/topologies/vm-distributed.config",
        )
        edc_ds = edc_cfg.get("DS_1_NAME") or "pionera-edc"
        edc_provider = edc_cfg.get("VM_PROVIDER_CONNECTORS", "citycounciledc").split(",")[0].strip()
        edc_consumer = edc_cfg.get("VM_CONSUMER_CONNECTORS", "companyedc").split(",")[0].strip()

        return {
            "inesdata": {
                "ds_name": inesdata_ds,
                "provider_short": inesdata_provider,
                "consumer_short": inesdata_consumer,
            },
            "edc": {
                "ds_name": edc_ds,
                "provider_short": edc_provider,
                "consumer_short": edc_consumer,
            },
        }
    ...
    def _sync_connector_routing_conf_vm_distributed(self, deployer_config, ds_name, ds_domain):
        ...
        Always generates maps for both INESData and EDC adapters so that re-running either
        adapter's level does not overwrite the other's routing configuration.
        """
        nodeport_default = str(deployer_config.get("K3S_INGRESS_HTTP_NODEPORT") or "31667").strip()
        provider_ip = str(deployer_config.get("VM_PROVIDER_IP") or "").strip()
        consumer_ip = str(deployer_config.get("VM_CONSUMER_IP") or "").strip()
        ...
        sibling = self._load_sibling_adapter_configs(deployer_config)
        inesdata = sibling.get("inesdata") or {}
        edc = sibling.get("edc") or {}

        inesdata_ds      = inesdata.get("ds_name") or ds_name
        inesdata_pshort  = inesdata.get("provider_short") or "citycouncil"
        inesdata_cshort  = inesdata.get("consumer_short") or "company"
        inesdata_phost   = f"conn-{inesdata_pshort}-{inesdata_ds}.{ds_domain}"
        inesdata_chost   = f"conn-{inesdata_cshort}-{inesdata_ds}.{ds_domain}"

        edc_ds      = edc.get("ds_name") or "pionera-edc"
        edc_pshort  = edc.get("provider_short") or "citycounciledc"
        edc_cshort  = edc.get("consumer_short") or "companyedc"
        edc_phost   = f"conn-{edc_pshort}-{edc_ds}.{ds_domain}"
        edc_chost   = f"conn-{edc_cshort}-{edc_ds}.{ds_domain}"

        conf = (
            f"# Generated by framework — vm-distributed connector cookie routing maps\n"
            f"# INESData connector interface routing\n"
            f"map $cookie_inesdata_connector $connector_host {{\n"
            f'    "{inesdata_cshort}"    {inesdata_chost};\n'
            f"    default      {inesdata_phost};\n"
            f"}}\n"
            f"map $cookie_inesdata_connector $connector_config_name {{\n"
            f'    "{inesdata_cshort}"    {inesdata_cshort};\n'
            f"    default      {inesdata_pshort};\n"
            f"}}\n"
            f"map $cookie_inesdata_connector $connector_backend {{\n"
            f'    "{inesdata_cshort}"    {consumer_ip}:{consumer_nodeport};\n'
            f"    default      {provider_ip}:{provider_nodeport};\n"
            f"}}\n"
            f"\n# EDC dashboard routing\n"
            f"map $cookie_edc_connector $edc_connector_host {{\n"
            f'    "{edc_cshort}"    {edc_chost};\n'
            f"    default      {edc_phost};\n"
            f"}}\n"
            f"map $cookie_edc_connector $edc_connector_backend {{\n"
            f'    "{edc_cshort}"    {consumer_ip}:{consumer_nodeport};\n'
            f"    default      {provider_ip}:{provider_nodeport};\n"
            f"}}\n"
        )
    ...
    def _sync_nginx_stream_proxy_vm_distributed(self):
        ...
        ok, err = _sudo_write_file(NGINX_STREAM_CONF, conf)
        if not ok:
            print(f"Warning: could not write {NGINX_STREAM_CONF}: {err}")
            return
        reload = subprocess.run(["sudo", "nginx", "-s", "reload"], capture_output=True, text=True)
        if reload.returncode == 0:
