...
evidencia de cierre del adapter EDC en topología `vm-distributed`. La evidencia
equivalente para INESData en `vm-distributed` debe tomarse desde
...
`vm-distributed` desde esta rama, `refactoring-vm-distributed-edc-ai`.
...
En esta rama de cierre, la evidencia actualizada de `edc` está disponible para
`vm-distributed`. Las evidencias de `local` y `vm-single` se mantienen en la
línea estable `refactoring-local-vm-single`.
