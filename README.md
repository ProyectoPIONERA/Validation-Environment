...
INESData y EDC en las topologías `local` y `vm-single`. Esta rama conserva la
evidencia de cierre del adapter INESData en topología `vm-distributed`. La
evidencia equivalente para EDC en `vm-distributed` debe tomarse desde
...
desde esta rama, `refactoring-vm-distributed-inesdata-ai`; y la evidencia EDC
...
En esta rama de cierre, la evidencia actualizada de `inesdata` está disponible
para `vm-distributed`. Las evidencias de `local` y `vm-single` se mantienen en
la línea estable `refactoring-local-vm-single`; la evidencia EDC en
`vm-distributed` se mantiene en `refactoring-vm-distributed-edc-ai`.
