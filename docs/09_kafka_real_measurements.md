...
- `KAFKA_EDC_KUBERNETES_EXEC_USE_TOPIC_OFFSETS`, por defecto desactivado en el backend `kubernetes-exec`. La suite consume desde el inicio de los topics de validación y filtra por los IDs generados por la propia prueba. Esto evita depender de consultas auxiliares de offset en brokers Kubernetes de un solo nodo. Si se activa, la suite vuelve a usar `GetOffsetShell`.
- `KAFKA_EDC_KUBERNETES_EXEC_TIMEOUT_SECONDS`, `KAFKA_EDC_STABILIZATION_GROUP_WAIT_SECONDS`, `KAFKA_EDC_STABILIZATION_PROBE_TIMEOUT_SECONDS`, `KAFKA_EDC_LATE_PROBE_CONFIRMATION_SECONDS` y `KAFKA_EDC_LATE_TRANSFER_CONFIRMATION_SECONDS` permiten ajustar ventanas de espera cuando el broker o el dataplane necesitan más tiempo. La validación sigue fallando si no se consumen todos los mensajes esperados.
- `KAFKA_EDC_PAIR_ATTEMPTS` y `KAFKA_EDC_PAIR_RETRY_SECONDS` controlan los reintentos del par proveedor-consumidor ante fallos transitorios de runtime Kafka. Cada reintento crea un flujo nuevo con topics nuevos; no reutiliza un resultado incompleto.
